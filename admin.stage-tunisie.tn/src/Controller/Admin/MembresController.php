        ...
        if ($form->isSubmitted() && $form->isValid()) {
           ...
           if($existe_membre)
           {
              $this->addFlash('error', 'Un compte est déjà enregistré avec cet e-mail');
               ...
               /*  $photoFile= $request->files->get('membres');
                
                if($photoFile['photo'] !== null)
                {
                    $originalFilename = $photoFile['photo']->getClientOriginalName();
                    $extensionImage = $photoFile['photo']->guessExtension();
                    $valideExtension = array('jpg', 'png', 'webp');
                    if (in_array($extensionImage, $valideExtension))
                    {
                        try {
                            $photoFile['photo']->move(
                                $this->getParameter('kernel.project_dir').'/public/uploads/membre/',
                                $originalFilename 
                            );
                            
                            $membre->setPhoto($originalFilename);
                          
                            $entityManager->persist( $membre);
                            $entityManager->flush();
                            
                            $newlFilename = "membre_". $membre->getId().'.'.$extensionImage;
                            rename($this->getParameter('kernel.project_dir').'/public/uploads/membre/'.$originalFilename, $this->getParameter('kernel.project_dir').'/public/uploads/membre/'.$newlFilename);
                            $membre->setPhoto($newlFilename);
                            $entityManager->flush();
                            $this->addFlash('success', 'Mise à jour effectuée avec succés.');
                            return $this->redirectToRoute('membres', [], Response::HTTP_SEE_OTHER);
